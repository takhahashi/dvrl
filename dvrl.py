...
class Dvrl(object):
  ...
  def train_dvrl(self, perf_metric):
    ...
    if self.flag_sgd:
      ...
      y_valid_temp = np.zeros((len(y_valid_hat)))
      y_valid_temp = y_valid_hat.argmax(axis=1)
      print("="*10)
      print(y_valid_temp)
      for i in len(y_valid_hat):
        y_valid_hat[i] = y_valid_temp[i]
    else:
      if self.problem == 'classification':
    ...
    if self.problem == 'classification':
      ...
      y_pred_diff = \
          np.abs(self.y_train_onehot + 1e-5 - y_train_valid_pred)/(self.y_train_onehot + 1e-5)
