...
# rmseを求めたいときのために追加１
def rmse(data_y, pred_y):
  # rmspeはなかなか使わない指標なのでrmseを実装
  from sklearn.metrics import mean_squared_error

  data_y = np.reshape(np.asarray(data_y), [len(data_y),])
  pred_y = np.reshape(pred_y, [len(pred_y),])

  output_pref=np.sqrt(mean_squared_error(data_y, pred_y))
...
def rmspe(data_y, pred_y):
  ...
  output_perf = np.sqrt(np.mean(((data_y + 1e-5 - pred_y)/(data_y + 1e-5))**2))
...
# kerasのニューラルネットを作成するための関数
# def get_keras_model():
  
# lightGBMではなくkerasのニューラルネットワークを使うために変更
def remove_high_low(dve_out, eval_model, x_train, y_train,
  ...
  for itt in range(num_bins):
    ...
    if len(np.unique(new_y_train)) > 1:
      ...
      # keras_model.fit(new_x_train, new_y_train)
      ...
      if perf_metric == 'auc':
        ...
        # y_valid_hat = keras_model.predict(x_valid)
        # y_test_hat = keras_model.predict(x_test)
      ...
      # rmseを求めたいときのために追加３
        ...
        temp_output[itt, 0] = rmse(y_valid, y_valid_hat)
        temp_output[itt, 1] = rmse(y_test, y_test_hat)       
    ...
    if len(np.unique(new_y_train)) > 1:
      ...
      # rmseを求めたいときのために追加４
        ...
        temp_output[num_bins + itt, 0] = rmse(y_valid, y_valid_hat)
        temp_output[num_bins + itt, 1] = rmse(y_test, y_test_hat)
  ...
  if plot:
    ...
    # どの指標で実験したかによってy軸は変更すべき
    if perf_metric == 'rmse':
      plt.ylabel('rmse', size=16)
    elif perf_metric == 'corrcoef':
      plt.ylabel('corrcoef', size=16)
    elif perf_metric == 'rmspe':
      plt.ylabel('rmspe', size=16)
...
def learn_with_baseline(eval_model, x_train, y_train, x_test, y_test,
  ...
  # rmseを求めるために追加６
    ...
    y_test_hat = eval_model.predict(x_test)
    output_perf = rmse(y_test, y_test_hat)
